...
# --- Show/hide state for goals selector ---
if "show_goals" not in st.session_state:
    st.session_state.show_goals = False

# --- Button to reveal the week-selection form ---
st.markdown("## Choose your macro goals")  # header above the button
if st.button("Choose Your Macro Goals"):
    st.session_state.show_goals = True

# --- When revealed, show the form and apply button ---
if st.session_state.show_goals:
    with st.form("week_form"):
        plan = st.selectbox(
            "Select your macro goal week:",
            presets_df.index.tolist(),
            index=1  # default to "Week 2 (Jul 1–7)"
        )
        apply_goals = st.form_submit_button("Apply Week")
    if apply_goals:
        ...
        st.dataframe(
            ref_df.rename(columns={
                ...
                ["Food","Unit","Protein (g)","Fat (g)",
                 "Carbs (g)","Fiber (g)","Net Carbs (g)","Calories per unit"]
            ]
        )
    ...
    st.info("Click 'Choose Your Macro Goals' to get started.")
```
