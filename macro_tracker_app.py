...
# --- Week selection form with Apply button, defaulting to Week 2 ---
with st.form("week_form"):
    plan = st.selectbox(
        "Select your macro goal week:",
        presets_df.index.tolist(),
        index=1  # zero-based: index=1 defaults to "Week 2"
    )
    apply_button = st.form_submit_button("Apply Week")
...
if apply_button:
    ...
    with st.form(key="menu_form", clear_on_submit=True):
        ...
        if st.form_submit_button("Add to Menu"):
            st.session_state.custom_foods.append({
                "Item": name, "unit": unit,
                "P/unit": p, "F/unit": f, "C/unit": c,
                "Fiber/unit": fiber, "Cal/unit": cal
            })
    ...
    if st.session_state.log:
        ...
        st.dataframe(n_df[[
            "id","Item","Amount","unit",
            "Protein_g","Fat_g","Net_Carbs_g","Fiber_g","Calories"
        ]])

        sel = st.selectbox(
            "Select ID to delete",
            options=n_df["id"].tolist(),
            key="del_id"
        )
        if st.button("Delete Entry", key="del_entry_button"):
            st.session_state.log = [
                e for e in st.session_state.log if e["id"] != sel
            ]
